...
from states.fsm import AdminAddAnime, AdminAddEpisode, AdminDeleteAnime, AdminDeleteEpisode, AdminAddFolder, AdminLinkAnime, AdminDeleteFolder, AdminListEpisodes, AdminEditAnime, AdminMassUpload, AdminEditFolder, AdminUnlinkAnime, AdminEditEpisode, AdminEditVoiceover, AdminDeleteVoiceover, AdminQuickEditEpisode
...
async def admin_list_anime(message: Message, session: AsyncSession):
    ...
    for a in animes:
        ...
        text += f"`{a.display_id}`. {html.escape(a.title)}{star}{folder_text}\n"
...
async def admin_list_episodes_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    text = "Доступные аниме:\n" + "\n".join([f"`{a.display_id}`. {html.escape(a.title)}{' 🌟' if getattr(a, 'is_4k', False) else ''}" for a in animes])
...
async def admin_list_episodes_process(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if not episodes:
        ...
        for ep in episodes:
            text += f"`{ep.id}`. Серия {ep.episode_number}\n"
...
async def add_anime_photo(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer(f"Аниме '{anime.title}' успешно добавлено! ID: `{anime.display_id}`", reply_markup=get_admin_menu(), parse_mode="Markdown")
    await state.clear()
...
async def edit_episode_anime_id(message: Message, state: FSMContext, session: AsyncSession):
    ...
    for vo in voiceovers:
        text += f"`{vo.id}`. {html.escape(vo.name)}\n"
...
async def link_anime_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    text = "Доступные папки:\n" + "\n".join([f"`{f.id}`. {html.escape(f.title)}" for f in folders])
